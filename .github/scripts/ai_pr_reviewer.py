...
2. Send the diff to Claude (claude-opus-4-5) with a structured expert prompt.
...
--------------------------------
ANTHROPIC_API_KEY   Anthropic API key (stored in GitHub Secrets).
...
def call_claude(user_message: str, api_key: str) -> dict[str, Any]:
    """Call Claude and parse the JSON review response."""
...
def format_github_review(review: dict[str, Any]) -> str:
    ...
    lines.append(
        ...
        f"using `{MODEL}`. Verify all findings before merging._"
    )
...
def main() -> None:
    ...
    print(f"[AI Review] Calling {MODEL}...")
    try:
        review = call_claude(user_message, api_key)
